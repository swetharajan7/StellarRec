...
export class SubmissionTracker {
  private logger = new Logger('SubmissionTracker');
   ...
   */
  async trackSubmission(data: SubmissionTrackingData): Promise<string> {
    try {
      const query = `
      ...
      // Get integration type from university
  ...
  /**
   * Get submissions by recommendation ID
   */
  async getSubmissionsByRecommendation(recommendationId: string): Promise<any[]> {
    try {
      const query = `
        SELECT 
          is_sub.*,
   ...
   */
  async getFailedSubmissions(recommendationId: string): Promise<any[]> {
    try {
      const query = `
        SELECT 
          is_sub.*,
   ...
   */
  async storeBulkSubmissionResult(result: BulkSubmissionResult): Promise<void> {
    try {
  ...
  /**
   * Get submissions requiring retry
   */
  async getSubmissionsForRetry(): Promise<any[]> {
    try {
      const query = `
        SELECT 
          is_sub.*,
   ...
   */
  async markForRetry(submissionId: string, retryAfter: Date, errorMessage?: string): Promise<void> {
    try {
      const query = `
        UPDATE integration_submissions 
        SET status = 'retry',
  ...
  /**
   * Get original request data (simplified version)
   */
  async getOriginalRequest(recommendationId: string): Promise<any> {
    try {
      // This would typically retrieve the original submission request
   ...
   */
  async cleanupOldSubmissions(olderThanDays: number = 90): Promise<number> {
    try {
      const query = `
